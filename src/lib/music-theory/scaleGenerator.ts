...
function midiToNoteParts(midi: number): { noteName: NoteName; octave: number } {
  ...
  for (let oct = 0; oct <= 2; oct++) {
    for (const iv of intervals) {
      ...
      for (let s = 0; s < 6; s++) {
        ...
        if (fret >= 1 && fret <= 12) {
          const { noteName, octave } = midiToNoteParts(midi);
...
export function getScaleLabel(key: KeyName, type: ScaleType): string {
  const labels: Record<ScaleType,string> = { major:'Major', minor:'Minor', jazz:'Jazz (Dorian)', blues:'Blues', arpeggio:'Arpeggio' };
